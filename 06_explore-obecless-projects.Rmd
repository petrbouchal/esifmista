...
if(file.exists(prj_geoname_validated_file)) {
  ...
  prj_geoname_checked2 <- prj_geoname_checked %>% 
    ...
    left_join(ids_and_names %>% 
                select(uzemi = obec_id, uzemi_nazev = obec_nazev) %>% 
                distinct()) %>% 
    mutate(prjtxt = if_else(kde == "anotace", prj_anotace, prj_nazev),
           orp_obec = str_detect(prjtxt, paste("(ORP|rozšířenou působností)", uzemi_nazev)),
           uzemi_is_sidlo = str_sub(p_sidlo_id, 6, 11) == uzemi_nazev,
           okres_obec = str_detect(prjtxt, paste("[Oo]kres([eu]?)", uzemi_nazev)))
