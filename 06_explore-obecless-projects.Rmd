...
if(file.exists(prj_geoname_validated_file)) {
  ...
  typuzemi <- zuj_nazvy %>% 
    select(obec = kod, uzemi_level = typuj) %>% 
  ...
  prj_geoname_for_check <- prj_geoname_matched %>%
    ...
    mutate(kde = "anotace") %>% 
    select(prj_id, kde, obec = geonames_in_prjanot) %>% 
    bind_rows(prj_geoname_matched %>% 
                unnest(cols = c(geonames_in_prjtitle)) %>% 
                select(prj_id, obec = geonames_in_prjtitle) %>% 
                mutate(kde = "nazev")) %>% 
    distinct(prj_id, obec, kde) %>% 
    left_join(dtl %>% 
                filter(typ == "id") %>% 
  ...
  with_progress({
    ...
    prj_geoname_checked <- prj_geoname_for_check %>% 
      mutate(obec_in_misto = future_pmap_lgl(list(obec,
                                                  value,
                                                  uzemi_level,
                                                  level),
                                             function(first, second, third, fourth) {
  ...
  prj_geoname_checked2 <- prj_geoname_checked %>% 
    ...
    left_join(ids_and_names %>% 
                select(obec = obec_id, obec_nazev) %>% 
                distinct()) %>% 
    mutate(prjtxt = if_else(kde == "anotace", prj_anotace, prj_nazev),
           orp_obec = str_detect(prjtxt, paste("(ORP|rozšířenou působností)", obec_nazev)),
           obec_is_sidlo = str_sub(p_sidlo_id, 6, 11) == obec,
...
write_parquet(dtl_noobec_diag, here::here("data-processed", "dtl-noobec-diag.parquet"))
```
...
diag_for_upset <- dtl_noobec_diag %>% 
  filter(op_id == "OP VVV") %>% 
...
```{r}
prj_id_unmatched <- dtl_noobec_diag %>% 
  filter(!matched) %>% 
  pull(prj_id)
```
