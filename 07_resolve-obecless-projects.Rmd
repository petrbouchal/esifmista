...
dtl_rozpadle_simple <- dtl_noobec %>% 
  ...
  select(prj_id, level, value = obec) %>% 
...
dtl_rozpadle_detekce <- dtl_geodetected %>% 
  ...
  select(prj_id, uzemi_nazev, rozpad_typ) %>% 
  left_join(dt_rozpad_key) %>% 
  left_join(rozpadac_detekovanych) %>% 
  drop_na(id) %>% 
  select(prj_id, value = obec, rozpad_typ) %>% 
  mutate(level = "obec")
```
...
dtl_rozpadle_sidla <- dt_rozpady %>% 
  filter(rozpad_typ == "obec_sidla") %>% 
  select(prj_id, rozpad_typ) %>% 
  left_join(dts) %>% 
  mutate(obec = p_sidlo_id, level = "obec") %>% 
  select(prj_id, value = obec, level, rozpad_typ)
```
...
dtl_rozpadle_mc <- dt_rozpady %>% 
  ...
  select(prj_id, zuj_nazev, p_nazev, rozpad_typ) %>%
  mutate(level = "zuj") %>% 
  left_join(ids_and_names %>% 
              select(zuj_nazev, zuj_id)) %>% 
  select(prj_id, rozpad_typ, level, value = zuj_id)
...
```{r praha}
dtl_rozpadle_praha <- dt_rozpady %>% 
  filter(rozpad_typ == "praha") %>% 
  mutate(level = "obec", value = "554782") %>% 
  select(prj_id, level, value, rozpad_typ)
```
...
dtl_rozpadle_plosne <- dtl_noobec %>% 
  ...
  mutate(level = "obec") %>% 
  select(prj_id, level, rozpad_typ, value = obec)
```
...
vysledek <- bind_rows(dtl_rozpadle_detekce,
  ...
  mutate(obec_puvod = "dovození obce-ZUJ pokud tato úroveň chyběla",
         ...
         nazev_orig = NA_character_,
         level_orig = NA_character_) %>% 
...
```{r slepeno-export}
write_parquet(vysledek %>% group_by(obec_puvod, 
                                    op_id, rozpad_typ) %>%
                select(prj_id, op_id, level, level_orig, 
                       rozpad_typ, rozpad_duvod, obec_puvod),
              here::here("data-processed", "dtl-obecless-rozpadnute-all", "all.parquet"),
              compression = "snappy")

ads <- open_dataset(here::here("data-processed", "dtl-obecless-rozpadnute-all"))

ads_grouped <- ads %>% group_by(rozpad_typ, op_id)

write_dataset(ads_grouped, here::here("data-processed", "dtl-obecless-rozpadnute"), format = "feather")
```

```{r}
fs::file_size(here::here("data-processed", "dtl-obecless-rozpadnute.parquet"))
```
