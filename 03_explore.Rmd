...
## Validita územní hierarchie {.tabset .tabset-pills}
...
prjs_geocheck <- data_with_geocheck %>% 
  group_by(op_id, prj_id) %>%
  summarise(all_ok = all(levels_ok, na.rm = T), .groups = "drop")
...
prjs_geostatus <- dt %>%
  ...
  mutate(multilevel = prj_id %in% multilevel_ids,
         geostatus = case_when(no_geo ~ "bez místa",
           ...
           as.factor() %>% fct_rev() %>% fct_relevel("bez místa"))

gst_bar <- prjs_geostatus %>% 
  ggplot() +
  scale_fill_manual(values = rev(c("grey", "darkgreen", "orange", "red"))) +
