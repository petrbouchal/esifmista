...
dt_geostructure_byprj <- dtl %>% 
  ...
  select(prj_id, level, level_num, op_id, value) %>% 
  group_by(prj_id, op_id) %>% 
  summarise(unique_units = length(unique(value)),
            n_levels = length(unique(level)),
            maxlevel = max(level_num), minlevel = min(level_num), 
            n_units = n(),
            .groups = "drop") %>% 
  mutate(across(c(maxlevel, minlevel), recode, !!!geolevels_recoder)) %>% 
...
dt_geostructure_bylvl <- dtl %>% 
  ...
  mutate(level_num = as.numeric(level)) %>% 
  group_by(prj_id, op_id, level, level_num) %>% 
  summarise(n_units = length(unique(value))) %>%
  group_by(prj_id, op_id) %>%
  mutate(min_level = level_num == min(level_num)) %>% 
