...
dtl_nh_checked <- plcnh %>% 
  ...
  rename(geo_id = value, geo_level = level, geo_id_short = shortid, geo_nazev = nazev) %>% 
  mutate(vybrat = FALSE,
...
```{r}
dtl_nh_resolved0 <- read_xlsx(here::here("data-input", "prj_nehierarchicke-resolved.xlsx"))
```
