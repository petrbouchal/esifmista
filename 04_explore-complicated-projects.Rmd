...
# Clustering podle kombinace míst a úrovní
...
cnts %>% 
  ...
  ggplot(aes(x = cnt)) +
  geom_histogram() +
  facet_grid(op_id ~ level)
```


```{r}
rownames(cnts_w) <- cnts_w$prj_id
cnts_w_n <- cnts_w %>% 
  ungroup() %>% 
  filter(op_id == "OP_ZP") %>% 
  select(-prj_id, -op_id) %>% 
...
```{r}
fviz_cluster(kmns, data = cnts_w_n, ggtheme = ptrr::theme_ptrr("both"))
```
...
dtl_with_geostatus_places <- dtl %>% 
  ...
  left_join(places_geostatus %>% 
              mutate(level = recode(level, orp = "rozobec")) %>% 
