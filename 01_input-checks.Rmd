...
```{r, message=F}
dtonpl_agent <- dt_noplace %>% 
  create_agent() %>% 
  col_vals_equal(all_na, FALSE) 

dtonpl_agent %>% 
  interrogate(extract_failed = T)
```
...
```{r noplace-op-zadatel}
dt_noplace %>% 
  count(op_id, p_nazev, p_sidlo_nazev)
```
...
dt_long_sidla %>% 
  ...
  create_agent() %>% 
  col_vals_equal(distinct_ids, 1, brief = "Each prj only has prijemce in one place ID") %>% 
  col_vals_equal(distinct_names, 1, brief = "Each prj only has prijemce in one place name") %>% 
  interrogate()
```
