...
dt %>% 
  ...
  distinct(g_zuj_id) 
```
Což jsou podle všeho hlavně městské části.

Oprava kódů ZÚJ: ke kratším na začátek přilepíme kód kraje

```{r zuj-fix}
dt_fixed_zuj <- dt %>% 
  mutate(g_zuj_id_short = str_sub(g_zuj_id, -6, -1)) %>% 
  left_join(csu_zuj %>% select(g_zuj_id_short = CHODNOTA2, CZNUTS)) %>% 
  mutate(g_zuj_id_new = paste0(CZNUTS, g_zuj_id_short),
         g_zuj_id_new = if_else(str_length(g_zuj_id) < 11,
                                g_zuj_id_new, g_zuj_id),
         g_zuj_id = g_zuj_id_new) %>% 
  select(-g_zuj_id_new, -g_zuj_id_short)
...
```{r revalidate}
dt_zujfix_validated <- dt_fixed_zuj %>% 
  mutate(valid_kraj = g_kraj_id %in% csu_kraj$CZNUTS,
         valid_obec = g_obec_id %in% obce_n$kod_n,
         valid_orp = g_rozobec_id %in% csu_orp$CHODNOTA,
         valid_okres = g_okres_id %in% csu_okres$CHODNOTA,
...
```{r fixed-write}
write_parquet(dt_fixed_zuj, here::here("data-processed", "misto-fixed.parquet"))
```

```{r}
dt_fixed_long <- make_long_geo(dt_fixed_zuj)
write_parquet(dt_fixed_long, here::here("data-processed", "misto-fixed-long-geo.parquet"))
```
