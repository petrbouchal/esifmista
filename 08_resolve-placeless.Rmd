...
dtl %>% 
  ...
  group_by(prj_id) %>% 
  mutate(maxlevel = min(as.numeric(level)),
         n_levels = length(unique(level))) %>% 
  filter(as.numeric(level) == maxlevel) %>% 
  group_by(op_id, prj_id, prj_nazev, p_nazev, level, n_levels) %>% 
...
dt_with_chuobce_reshaped <- dt_with_chuobce %>% 
  mutate(level = "obec", level_orig = "chkonp") %>% 
  rename_with(~str_remove(., "zuj_")) %>% 
  select(-starts_with("chkonp"))
