...
obce_v_chu <- zuj_geo %>% 
  select(kod, nazev) %>% 
  mutate(has_chu = map(geometry, ~st_intersects(.x, chu_geo %>% 
                                                  st_transform(5514))))
```

```{r obce-s-chu}
obce_s_chu <- obce_v_chu %>% 
...
```{r obce-s-chu-mapview}
mapview::mapview(obce_s_chu, zcol = "NAZEV") %>% 
...
obce_s_chu_to_join <- obce_s_chu %>% 
  st_set_geometry(NULL) %>% 
  select(obec_kod = kod, obec_nazev = nazev, 
